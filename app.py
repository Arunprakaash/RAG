...
try:
    ipc_query_engine, bns_query_engine = get_query_engines()
except:
   print("no vectors")
...
@cl.on_chat_start
async def factory():
    cl.user_session.set("query_engine", ipc_query_engine)
    cl.user_session.set("bns_query_engine",bns_query_engine)

    tools = [
        Tool(
            name="IPC-tool",
            func=query_engine_func,
            description="Useful for when you need find information about Indian Penal Code Section (IPC).",
        ),
        Tool(
            name="BNS-tool",
            func=bns_query,
